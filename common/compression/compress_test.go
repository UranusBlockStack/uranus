...
func TestDecodingCycle(t *testing.T) {
	...
	for i, tt := range tests {
		data, err := utils.Decode(tt.input)
		if err != nil {
			t.Errorf("test %d: failed to decode data: %v", i, err)
		}
...
func TestCompression(t *testing.T) {
	...
	in, err := utils.Decode("0x4912385c0e7b64000000")
	if err != nil {
		t.Errorf("failed to decode data: %v", err)
	}
	out, err := utils.Decode("0x80fe4912385c0e7b64")
	if err != nil {
		t.Errorf("tfailed to decode data: %v", err)
	}
	...
	in, err = utils.Decode("0xdf7070533534333636313639343638373532313536346c1bc33339343837313070706336343035336336346c65fefb3930393233383838ac2f65fefb")
	if err != nil {
		t.Errorf("failed to decode data: %v", err)
	}
	out, err = utils.Decode("0xdf7070533534333636313639343638373532313536346c1bc33339343837313070706336343035336336346c65fefb3930393233383838ac2f65fefb")
	if err != nil {
		t.Errorf("failed to decode data: %v", err)
	}
	if data := CompressBytes(in); !bytes.Equal(data, out) {
