...
import (
	...
	"github.com/UranusBlockStack/uranus/p2p/nat"
...
func ListenUDP(c conn, cfg Config) (*Table, error) {
	...
	log.Infof("UDP listener up self %s", tab.self)
	return tab, nil
}
...
func (t *udp) findnode(toid NodeID, toaddr *net.UDPAddr, target NodeID) ([]*Node, error) {
	...
	errc := t.pending(toid, neighborsPacket, func(r interface{}) bool {
		...
		for _, rn := range reply.Nodes {
			...
			if err != nil {
				log.Infof("Invalid neighbor node received ip %v addr %v err %v", rn.IP, toaddr, err)
				continue
			}
...
func (t *udp) handlePacket(from *net.UDPAddr, buf []byte) error {
	...
	if err != nil {
		log.Debugf("Bad discv4 packet addr %v err %v", from, err)
		return err
	}
