...
func (s *Server) WebsocketHandler(allowedOrigins []string) http.Handler {
	return websocket.Server{
		...
		Handler: func(conn *websocket.Conn) {
			codec := newWebsocketServerCodec(conn)
			s.ServeCodec(codec)
		},
	}
}
...
func newWebsocketServerCodec(conn *websocket.Conn) ServerCodec {
	...
	return NewJSONServerCodec(rpcconn, encoder, decoder)
}

func newWebsocketClientCodec(conn *websocket.Conn) ClientCodec {
	// Create a custom encode/decode pair to enforce payload size and number encoding
	conn.MaxPayloadBytes = maxHTTPRequestContentLength
	encoder := func(v interface{}) error {
		return websocketJSONCodec.Send(conn, v)
	...
	return NewJSONClientCodec(rpcconn, encoder, decoder)
}
...
func DialWebsocket(ctx context.Context, endpoint, origin string) (*Client, error) {
	...
	codec := newWebsocketClientCodec(conn)
