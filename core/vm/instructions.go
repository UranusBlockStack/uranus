...
func opSignExtend(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	if back.Cmp(big.NewInt(31)) < 0 {
		...
		num := stack.pop()
		mask := back.Lsh(utils.Big1, bit)
		mask.Sub(mask, utils.Big1)
...
func opSha3(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	if evm.vmConfig.EnablePreimageRecording {
		evm.StateDB.AddPreimage(utils.BytesToHash(hash), data)
	}
...
func opBalance(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	slot := stack.peek()
	slot.Set(evm.StateDB.GetBalance(utils.BigToAddress(slot)))
	return nil, nil
}
...
func opExtCodeSize(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	slot := stack.peek()
	slot.SetUint64(uint64(evm.StateDB.GetCodeSize(utils.BigToAddress(slot))))
...
func opExtCodeCopy(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	var (
		addr       = utils.BigToAddress(stack.pop())
		memOffset  = stack.pop()
		codeOffset = stack.pop()
		length     = stack.pop()
	)
...
func opBlockhash(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	n := evm.interpreter.intPool.get().Sub(evm.BlockNumber, utils.Big257)
...
func opCall(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
	...
	toAddr := utils.BigToAddress(addr)
...
func makeLog(size int) executionFunc {
	return func(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
		topics := make([]utils.Hash, size)
		mStart, mSize := stack.pop(), stack.pop()
		for i := 0; i < size; i++ {
			topics[i] = utils.BigToHash(stack.pop())
		}
		...
		evm.StateDB.AddLog(&types.Log{
			...
			BlockHeight: evm.BlockNumber.Uint64(),
		})
...
func makePush(size uint64, pushByteSize int) executionFunc {
	return func(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error) {
		...
		stack.push(integer.SetBytes(utils.RightPadBytes(contract.Code[startMin:endMin], pushByteSize)))
