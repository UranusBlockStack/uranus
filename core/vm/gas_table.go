...
func gasSStore(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	var (
		...
		val  = evm.StateDB.GetState(contract.Address(), utils.BigToHash(x))
	)
	// This checks for 3 scenario's and calculates gas accordingly
	// 1. From a zero-value address to a non-zero value         (NEW VALUE)
	// 2. From a non-zero value address to a zero-value address (DELETE)
	// 3. From a non-zero to a non-zero                         (CHANGE)
	if utils.EmptyHash(val) && !utils.EmptyHash(utils.BigToHash(y)) {
		// 0 => non 0
		return params.SstoreSetGas, nil
	} else if !utils.EmptyHash(val) && utils.EmptyHash(utils.BigToHash(y)) {
...
func gasCall(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	var (
		...
		address        = utils.BigToAddress(stack.Back(1))
		eip158         = params.IsEIP158(evm.BlockNumber)
	)
	if eip158 {
...
func gasSuicide(gt params.GasTable, evm *EVM, contract *Contract, stack *Stack, mem *Memory, memorySize uint64) (uint64, error) {
	...
	if params.IsEIP150(evm.BlockNumber) {
		gas = gt.Suicide
		var (
			address = utils.BigToAddress(stack.Back(0))
			eip158  = params.IsEIP158(evm.BlockNumber)
		)
