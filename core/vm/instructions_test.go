...
func testTwoOperandOp(t *testing.T, tests []twoOperandTest, opFn func(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error)) {
	...
	for i, test := range tests {
		x := new(big.Int).SetBytes(utils.HexToBytes(test.x))
		shift := new(big.Int).SetBytes(utils.HexToBytes(test.y))
		expected := new(big.Int).SetBytes(utils.HexToBytes(test.expected))
		stack.push(x)
		stack.push(shift)
...
func TestByteOp(t *testing.T) {
	...
	for _, test := range tests {
		val := new(big.Int).SetBytes(utils.HexToBytes(test.v))
...
func opBenchmark(bench *testing.B, op func(pc *uint64, evm *EVM, contract *Contract, memory *Memory, stack *Stack) ([]byte, error), args ...string) {
	...
	for i, arg := range args {
		byteArgs[i] = utils.HexToBytes(arg)
	}
	pc := uint64(0)
	bench.ResetTimer()
	for i := 0; i < bench.N; i++ {
