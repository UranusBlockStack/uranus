...
func makeTestState() (Database, utils.Hash, []*testAccount) {
	...
	for i := byte(0); i < 96; i++ {
		obj := state.GetOrNewStateObject(utils.BytesToAddress([]byte{i}))
		acc := &testAccount{address: utils.BytesToAddress([]byte{i})}
...
// account array.
func checkStateAccounts(t *testing.T, db ldb.Database, root utils.Hash, accounts []*testAccount) {
...
func checkTrieConsistency(db ldb.Database, root utils.Hash) error {
	if v, _ := db.Get(root[:]); v == nil {
		return nil // Consider a non existent state consistent.
	}
	trie, err := mtp.New(root, mtp.NewDatabase(db))
	if err != nil {
		return err
	}
...
func checkStateConsistency(db ldb.Database, root utils.Hash) error {
...
func testIterativeStateSync(t *testing.T, batch int) {
	...
	for len(queue) > 0 {
		results := make([]mtp.SyncResult, len(queue))
		for i, hash := range queue {
			data, err := srcDb.TrieDB().Node(hash)
			if err != nil {
				t.Fatalf("failed to retrieve node data for %x", hash)
			}
			results[i] = mtp.SyncResult{Hash: hash, Data: data}
		}
...
func TestIterativeDelayedStateSync(t *testing.T) {
	...
	for len(queue) > 0 {
		...
		results := make([]mtp.SyncResult, len(queue)/2+1)
...
func testIterativeRandomStateSync(t *testing.T, batch int) {
	...
	for len(queue) > 0 {
		...
		results := make([]mtp.SyncResult, 0, len(queue))
		for hash := range queue {
			data, err := srcDb.TrieDB().Node(hash)
			if err != nil {
				t.Fatalf("failed to retrieve node data for %x", hash)
			}
			results = append(results, mtp.SyncResult{Hash: hash, Data: data})
		}
...
func TestIterativeRandomDelayedStateSync(t *testing.T) {
	...
	for len(queue) > 0 {
		...
		results := make([]mtp.SyncResult, 0, len(queue)/2+1)
		for hash := range queue {
			delete(queue, hash)
